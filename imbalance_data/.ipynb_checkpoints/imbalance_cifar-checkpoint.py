...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __init__(self, root, imb_type='exp', imb_factor=0.01, rand_number=0, weighted_alpha=0.2,
    ...
    def __getitem__(self, index):
        ...
        # transform[0] : classification branch
        # transform[1],[2] : contrastive branch
...
#     def __init__(self, mode, cfg, root = '../../data', imb_type='exp',
#                  transform=None, target_transform=None, download=True):
#         train = True if mode == "train" else False
#         super(IMBALANCECIFAR10, self).__init__(root, train, transform, target_transform, download)
#         self.cfg = cfg
#         self.train = train
#         self.dual_sample = True if cfg.dual_sample and self.train else False
#         rand_number = cfg.rand_num
#         if self.train:
#             np.random.seed(rand_number)
...
#         if self.dual_sample or (self.cfg.sampler_type in ['balance', 'reverse'] and self.train):
...
#             tuple: (image, target) where target is index of the target class.
#         """
#         if self.cfg.sampler_type in ['balance', 'reverse'] and self.train:
#             assert self.cfg.sampler_type in ["balance", "reverse"]
#             if  self.cfg.sampler_type == "balance":
#                 sample_class = random.randint(0, self.cls_num - 1)
#             elif self.cfg.sampler_type == "reverse":
...
#         if self.dual_sample:
#             if self.cfg.dual_sampler_type == "reverse":
#                 sample_class = self.sample_class_index_by_weight()
#                 sample_indexes = self.class_dict[sample_class]
#                 sample_index = random.choice(sample_indexes)
#             elif self.cfg.dual_sampler_type == "balance":
#                 sample_class = random.randint(0, self.cls_num-1)
#                 sample_indexes = self.class_dict[sample_class]
#                 sample_index = random.choice(sample_indexes)
#             elif self.cfg.dual_sampler_type == "uniform":
