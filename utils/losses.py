...
class SupConLoss_proto(nn.Module):
    def __init__(self, cls_num_list, temperature=0.07, contrast_mode='all', base_temperature=0.07):
        super(SupConLoss_proto, self).__init__()
        self.temperature = temperature
        self.contrast_mode = contrast_mode
        self.base_temperature = base_temperature
    ...
    def forward(self, label_center, centers, features, labels=None, mask=None):
        ...
        if labels is not None and mask is not None:
            ...
            if labels.shape[0] != batch_size:
                raise ValueError('Num of labels does not match num of features')
            mask = torch.eq(labels, label_center.T).float().to(device)
        else:
            mask = mask.float().to(device)
        ...
        anchor_dot_contrast = torch.div(
            torch.matmul(anchor_feature, centers.T),
            self.temperature)
        anchor_dot_contrast = anchor_dot_contrast.repeat(1,2)
        
        # tile mask
        mask = mask.repeat(anchor_count, contrast_count)

        # weight_mask = self.cls_weight.repeat(features.shape[0]*anchor_count,contrast_count)
        ...
        # compute mean of log-likelihood over positive
        mean_log_prob_pos = (mask*log_prob).sum(1)/(mask.sum(1))
        # mean_log_prob_pos = (weight_mask*log_prob).sum(1)/(weight_mask.sum(1))
